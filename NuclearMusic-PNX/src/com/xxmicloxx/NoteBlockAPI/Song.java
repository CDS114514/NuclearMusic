    ...
    public void clearMultiNote() {
        ...
        for (int tick = 0; tick < length; tick++) {
            ...
            for (Layer layer : this.layerHashMap.values()) {
                ...
                if (note != null) {
                    noteMap.putIfAbsent(note.getInstrument(false, false), new ArrayList<>());
                    if (noteMap.get(note.getInstrument(false, false)).stream().anyMatch(key -> note.getKey() == key)) {
                        layer.getHashMap().remove(tick);
                        Server.getInstance().getLogger().info("[" + getTitle() + "] 已移除重复Note: " + tick + " - " + note.getSoundEnum(false, false) + " - " + note.getKey());
                    } else {
                        noteMap.get(note.getInstrument(false, false)).add(note.getKey());
                    }
                }
            }
        }
